...
import (
	...
	videoEndpoints "github.com/viki-org/gokit-template/pkg/endpoints/video"
	sampleServices "github.com/viki-org/gokit-template/pkg/services/sample"
	videoServices "github.com/viki-org/gokit-template/pkg/services/video"
	sampleStores "github.com/viki-org/gokit-template/pkg/stores/sample"
	videoStores "github.com/viki-org/gokit-template/pkg/stores/video"
	...
	videoHTTPTransport "github.com/viki-org/gokit-template/pkg/transports/http/video"
)
...
func (a *App) Initialize(configs ...AppConfig) error {
	...
	videoStore := videoStores.NewDBStore(a.DB, a.Logger)
	videoService := videoServices.NewAPIService(videoStore, a.Logger)
	videoEndpoint := videoEndpoints.MakeServerEndpoints(videoService)
	...
	a.Router.Methods("GET").Path("/videos/{videoID}.json").Handler(httptransport.NewServer(
		videoEndpoint.RetrieveEndpoint,
		videoHTTPTransport.DecodeRetrieveRequest,
		svchttphandlers.EncodeStatusOKResponse,
		options...,
	))

	a.Router.Methods("POST").Path("/videos.json").Handler(httptransport.NewServer(
		videoEndpoint.CreateEndpoint,
		videoHTTPTransport.DecodeCreateRequest,
		svchttphandlers.EncodeStatusCreatedResponse,
		options...,
	))
